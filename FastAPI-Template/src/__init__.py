...
from api.openapi_patch import patch_openapi_schema
...
async def lifespan(app: FastAPI):
    ...
    await init_data()
    await poker_manager.start()
    try:
        yield
    finally:
        await poker_manager.stop()
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        docs_url=None,
        redoc_url=None,
        # We serve a custom /openapi.json endpoint (patched schema). Disable the
        # built-in OpenAPI route to avoid path collision.
        openapi_url=None,
    ...
    async def custom_swagger_ui_html():  # Swagger 基础认证已临时关闭
    ...
    async def redoc_html():  # Swagger 基础认证已临时关闭
    ...
    async def get_open_api_endpoint():  # Swagger 基础认证已临时关闭
        ...
        return patch_openapi_schema(openapi_schema)

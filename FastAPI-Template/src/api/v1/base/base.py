...
from schemas.response import CurrentUserResponse, ResponseBase, TokenResponse
...
from services.subscription_tier import get_user_effective_tier, tier_to_name
from api.openapi_examples import ok_response_example


class AdaptiveEnvConfig(StarletteConfig):
    def _read_file(self, file_name, encoding="utf-8"):
...
async def refresh_access_token(request: Request, refresh_request: RefreshTokenRequest):
    ...
    """
    try:
        # 若 refresh token 已注销，直接拒绝
        if await is_refresh_token_revoked(refresh_request.refresh_token):
            raise HTTPException(
                status_code=401, detail="i18n:auth.token_invalid_or_expired"
            )
...
@router.post(
    ...
    responses=ok_response_example(None),
)
async def logout(
    body: LogoutRequest = Body(default=LogoutRequest(), description="退出登录参数"),
    current_user: User = DependAuth,
):
    """退出登录

    - 若传入 refresh_token：服务端会将其加入黑名单，后续刷新将返回 401。
    - 不传 refresh_token：表示客户端仅做本地清理（服务端无法撤销短期 access token）。
    """

    if body.refresh_token:
        # 尝试撤销 refresh token（无 Redis 时会落到进程内兜底）
        try:
            payload = verify_token(body.refresh_token, token_type="refresh")
            if int(payload.user_id) != int(current_user.id):
                raise HTTPException(status_code=403, detail="i18n:common.forbidden")
        except jwt.ExpiredSignatureError:
            # 已过期则无需撤销，仍视为登出成功
            payload = None
        except jwt.InvalidTokenError:
            raise HTTPException(status_code=401, detail="i18n:auth.failed")

        try:
            await revoke_refresh_token(body.refresh_token)
        except Exception:
            # Best-effort: 不因撤销失败影响登出成功语义
            pass

    # 登出时顺手清理用户相关缓存
    try:
        await clear_user_cache(int(current_user.id))
    except Exception:
        pass

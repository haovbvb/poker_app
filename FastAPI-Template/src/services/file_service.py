...
class FileService:
    ...
    async def upload_file(self, file: UploadFile, user_id: int) -> Success:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="i18n:file.upload_failed"
            ) from e

    def _validate_file_security(self, file: UploadFile) -> None:
        """验证文件安全性"""
        if not file.filename:
            raise HTTPException(status_code=400, detail="i18n:file.filename_empty")
        ...
        if file_ext in DANGEROUS_EXTENSIONS:
            raise HTTPException(
                status_code=400,
                detail={
                    "i18n_key": "file.type_not_allowed",
                    "params": {"ext": file_ext},
                },
            )
        ...
        if file_ext and file_ext not in ALLOWED_EXTENSIONS:
            raise HTTPException(
                status_code=400,
                detail={
                    "i18n_key": "file.type_not_supported",
                    "params": {
                        "ext": file_ext,
    ...
    async def _read_and_validate_file(self, file: UploadFile) -> bytes:
        ...
        if len(content) > MAX_FILE_SIZE:
            raise HTTPException(
                status_code=400,
                detail={
                    "i18n_key": "file.size_exceeded",
                    "params": {"limit_mb": MAX_FILE_SIZE // (1024 * 1024)},
                },
            )

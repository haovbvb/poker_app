...
class LocaleMiddleware(BaseHTTPMiddleware):
    ...
    Priority:
    1) Query param `lang` (e.g. `?lang=zh` / `?lang=en`)
    2) Header `Accept-Language` (e.g. `zh-CN,zh;q=0.9,en;q=0.8`)
    ...
    @staticmethod
    def _from_accept_language(header_value: str | None) -> str:
        if not header_value:
            return "en"
        # Take the first tag before ',' and strip q-value
        first = header_value.split(",", 1)[0].strip()
        first = first.split(";", 1)[0].strip()
        return LocaleMiddleware._normalize(first)

    async def dispatch(
        self, request: Request, call_next: RequestResponseEndpoint
    ) -> Response:
        lang_param = request.query_params.get("lang")
        lang = self._normalize(lang_param)
        if not lang_param:
            lang = self._from_accept_language(request.headers.get("accept-language"))
...
class HttpAuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def get_request_log(self, request: Request, response: Response) -> dict:
        ...
        data: dict = {
            ...
            # Ensure required AuditLog fields are never null
            "module": "",
            "summary": "",
        }
        # 路由信息
        ...
        for route in app.routes:
            if (
                ...
                data["module"] = ",".join(route.tags or [])
    ...
    async def after_request(
        ...
        if request.method in self.methods:
            ...
            try:
                ...
                ).warning("AuditLog create failed")

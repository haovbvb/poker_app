    ...
    async def is_authed(
        ...
        try:
            ...
            if token is None or not token.credentials:
                raise HTTPException(status_code=401, detail="i18n:auth.missing_token")
            ...
            raise HTTPException(status_code=401, detail="i18n:auth.failed") from e
...
class PermissionControl:
    ...
    async def has_permission(
        ...
        if not roles:
            raise HTTPException(status_code=403, detail="i18n:auth.user_not_bound_role")
...
class AgentPermissionControl:
    ...
    async def has_agent_permission(
        ...
        if not agent_id:
            raise HTTPException(status_code=400, detail="i18n:agent.id_missing")

        try:
            agent_id = int(agent_id)
        except (ValueError, TypeError) as e:
            raise HTTPException(status_code=400, detail="i18n:agent.id_invalid") from e

        # 获取用户角色
        roles: list[Role] = await current_user.roles.all()
        if not roles:
            raise HTTPException(status_code=403, detail="i18n:agent.role_not_bound")
        ...
        raise HTTPException(status_code=403, detail="i18n:agent.access_denied")

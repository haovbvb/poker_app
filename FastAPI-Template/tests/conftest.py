...
# Force test environment regardless of the caller's shell env.
# Using setdefault here is fragile because local dev shells often export
# APP_ENV=development / DEBUG=1, which makes pytest run in dev mode.
...
os.environ["APP_TITLE"] = os.environ.get("APP_TITLE", "FastAPI Backend Template")
os.environ["PROJECT_NAME"] = os.environ.get("PROJECT_NAME", "FastAPI Backend Template")

# Ensure SECRET_KEY always passes validation and is deterministic in tests.
...
try:  # pragma: no cover - fallback for environments without pytest-asyncio
    ...
    try:
        ...
        warnings.warn("Installed pytest-asyncio dynamically to enable async fixtures.")
...
async def reset_poker_state():
    """Reset in-memory poker tables for each test."""
    from poker import poker_manager

    await poker_manager.reset_for_tests()
    yield
...
async def clean_database():
    ...
    try:
        # 清理所有测试数据（忽略外键约束错误）
        await MessageUserState.all().delete()
        await Message.all().delete()
        await BankruptcyReliefClaim.all().delete()
        await DailyRewardClaim.all().delete()
        await UserWallet.all().delete()
